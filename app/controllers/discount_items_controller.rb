class DiscountItemsController < ApplicationController
  ...
  def create
    # puts "DiscountItemsController.create"
    @discount_item = @cart.add_discount(@discount)

    respond_to do |format|
      if @discount_item.save
        format.html { redirect_to @cart, notice: 'Discount item was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @discount_item.update(discount_item_params)
        format.html { redirect_to @discount_item.cart, notice: 'Discount item was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to @cart, notice: 'Discount item was successfully destroyed.' }
  ...
  private
    ...
    def set_cart
      @cart = Cart.find(discount_item_params[:cart_id])
    end

    def set_discount
      @discount = Discount.find(discount_item_params[:discount_id])
    end
end
